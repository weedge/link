...
//====add the Link info modified by wy======
  bool usedLink;
  int windowCount;
  std::vector<int> windowSizes;
  std::string candidateLinkFile;
  bool usedCandLink;
//=========================================
...
//========================add the Link info modified by wy============================
	usedLink = (ParamGetString("usedLink", "false") == "true");
	...
	usedCandLink = (ParamGetString("usedCandLink", "false") == "true");
//=============================================================================
  }
};
...
int AppMain(int argc, char * argv[]) {
  ...
  indri::utility::IndriTimer indexTimer;
  indexTimer.start();
  ...
  if (LocalParameter::indexType == "indri") {
    indexer = new lemur::parse::IndriTextHandler(LocalParameter::index,
		...
		//如果usedLink = true; 建立Keyfile类型索引和Link类型索引文件。
	  index = new link::api::InvLinkIndex(LocalParameter::index, LocalParameter::windowSizes,
								LocalParameter::candidateLinkFile,LocalParameter::usedLink,
								LocalParameter::usedCandLink,LocalParameter::memory);

	  indexer = new lemur::parse::KeyfileTextHandler(index,LocalParameter::countStopWords);
  ...
  indexTimer.stop();
  std::cerr << "total execution time(minute:second.microsecond)";
  indexTimer.printElapsedMicroseconds(std::cerr);
  std::cerr << std::endl;
